---
name: wgl-theatrejs-editor
description: Track 9 rail (webgl-motion-stack) — Theatre.js expertise — a visual, scrubbable animation editor that binds directly to production code (Three.js/R3F meshes, materials, cameras, and custom shader uniforms). Covers project/sheet/sequence architecture, binding timeline properties to WebGL objects and uniforms, production deployment without shipping the studio GUI, and triggering sequences from scroll/click/user data. Use whenever the user wants a draggable/scrubbable timeline for a 3D scene, wants a designer to tweak WebGL animation without touching code, or mentions Theatre.js.
version: 0.1.0
...
Binding is push-based: Theatre.js calls `onValuesChange` on every tick where a tracked value changed — write those values directly into Three.js properties or shader uniforms, mutating in place (same allocation-free discipline as `de5-lowlevel-js`, since this runs inside a render loop).
...
- For R3F integration, drive the binding inside `useFrame`, not `useEffect`/`setState` — see `de3-threejs-r3f` for why per-frame `setState` in React kills performance.
...
This is a common real pairing: **GSAP/ScrollTrigger drives scroll progress → that progress scrubs a Theatre.js sequence position → the sequence drives Three.js object properties.** See `wgl-gsap-motion` for the ScrollTrigger half.
...
| Simple scroll-triggered DOM/property tweens, code-authored | GSAP alone (`wgl-gsap-motion`) — lighter, no extra dependency |
