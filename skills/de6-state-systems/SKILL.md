---
name: de6-state-systems
description: Track 6 rail — advanced state systems. Managing complex interactive state without wasteful re-renders — the transient-vs-discrete state split, Zustand selector subscriptions, XState finite state machines for gesture/animation states, Signals for fine-grained reactivity, and re-render audit techniques. Use whenever wiring interaction state (drag/hover/open states), choosing between useState/Zustand/XState/Signals, fixing components that re-render during animations or gestures, or debugging interactions that get "stuck" between states.
version: 0.1.0
