...
- `CustomEase.create('id', 'M0,0 C0.2,0 0,1 1,1')` — hand-drawn curves matching a reference site's exact feel (pair with [de1-aesthetic-deconstruction](de1-aesthetic-deconstruction.md) to extract the curve from a reference).
- `CustomBounce` / `CustomWiggle` — parametric physical bounce/wiggle instead of faking it with keyframes.
- For true spring physics (interruptible, velocity-aware), GSAP eases are not enough — hand off to a real spring solver ([de4-spring-damper](de4-spring-damper.md)) and drive GSAP-managed properties from it, or use GSAP's own `Physics2DPlugin` for simple cases.
...
- Animate only `x`, `y`, `scale`, `rotation`, `opacity` on the DOM (composite-only, GPU-accelerated) — never `top`/`left`/`width`/`height` (see [de5-critical-rendering-path](de5-critical-rendering-path.md) for why).
