---
name: de5-canvas-2d
description: Track 5 rail — the HTML5 Canvas 2D API at production quality. High-DPI (Retina) setup with devicePixelRatio, pre-rendering to offscreen canvases, particle-system batching with typed arrays, clipping/masking/composite operations, motion trails, and knowing when Canvas beats DOM. Use whenever building canvas animations, particle effects, generative visuals, custom drawing surfaces, or whenever canvas output looks blurry, or hundreds of animated DOM nodes are melting the frame budget.
version: 0.1.0
