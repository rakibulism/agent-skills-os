---
name: wgl-draw-call-batching
description: Track 9 rail (webgl-motion-stack) — Deep draw-call reduction — why draw calls are expensive (CPU-to-GPU driver overhead, not just GPU load), instanced attributes for per-instance variation in a single call, dynamic instance updates without CPU bottlenecks, texture atlases/arrays, frustum culling, spatial-grid hierarchical batching, LOD transitions, and when NOT to instance. Applies across Three.js/R3F, PixiJS, and raw WebGL/WebGPU. Use whenever frame rate degrades as object count grows, the user has hundreds/thousands of similar objects (particles, trees, crowd, cards, tiles), or is deciding whether to batch/instance something.
version: 0.1.0
...
- Write directly into the `InstancedMesh.instanceMatrix.array` (a typed array) rather than calling `setMatrixAt` inside a loop that also allocates a temp `Matrix4` per iteration — allocate the temp matrix **once outside the loop** and reuse it (`de5-lowlevel-js` discipline).
...
- **PixiJS:** the same driver-overhead logic is why `ParticleContainer` and texture-atlas batching exist — see `wgl-pixijs-2d-engine`.
